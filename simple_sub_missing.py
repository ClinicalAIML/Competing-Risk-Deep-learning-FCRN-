...
import sys
sys.path.insert(0, '.')
from model.network_miss import StoNet_Survival_Sub
from model.training_missing import training_survival_sub
from data_sub import sub_competing_missing
...
parser.add_argument('--num_sim', default=8, type=int, help='number of simulation runs')
...
parser.add_argument('--impute_lr_miss', default=1e-2, type=float,
...
parser.add_argument('--mh_step', default=2, type=int, help='number of SGHMC step for imputation')
...
def fit_and_evaluate_Causal_StoNet(data_seed, save_results=True):
    ...
    train_set = sub_competing_missing(train_path, args.interval_length, args.event)
    test_set = sub_competing_missing(test_path, args.interval_length, args.event)
    ...
    base_path = os.path.join(f'./competing_risk_results/subdistribute/missing_simple_inter_{args.miss_pattern}_{args.miss_rate}/event_{args.event}/exp_{args.num_sim}', str(data_seed))
    ...
    for prune_seed in range(num_seed):
        ...
        with torch.no_grad():
            ...
            count = 0
            for target, weight, x, _, actual_time, test_time, test_indicator in val_data:
