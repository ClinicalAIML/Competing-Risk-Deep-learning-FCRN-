...
import sys
sys.path.insert(0, '.')
from model.network_fda import StoNet_Survival_FDA_sub
from model.training import training_survival_fda_sub
from data_fda import sub_competing_complete
...
parser.add_argument('--num_sim', default=6, type=int, help='number of simulation runs')
...
def fit_and_evaluate_Causal_StoNet(data_seed, save_results=True):
    ...
    train_set = sub_competing_complete(train_path, train_fda_path, args.interval_length, args.event, args.view)
    test_set = sub_competing_complete(test_path, test_fda_path, args.interval_length, args.event, args.view)
    ...
    base_path = os.path.join(f'./competing_risk_results/subdistribute/complete_simple_fda_view3/event_{args.event}/exp_{args.num_sim}', str(data_seed))
    ...
    for prune_seed in range(num_seed):
        ...
        net = StoNet_Survival_FDA_sub(**net_args)
        net.to(device)
        ...
        # train
        print("Train")
        output_train = training_survival_fda_sub(mode="train",
                                net=net,
        ...
        with torch.no_grad():
            ...
            count = 0
            for target, weight, x, x_fda, actual_time, test_time, test_indicator in val_data:
