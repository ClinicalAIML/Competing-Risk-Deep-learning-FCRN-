...
import sys
sys.path.insert(0, '.')
from model.network import StoNet_Survival
from model.training import training_survival
from data import competing_risk_inter2
...
# simulation setting
parser.add_argument('--data_num', default=4, type=int, help='number of simulation')
parser.add_argument('--num_sim', default=3, type=int, help='number of simulation runs')
...
def fit_and_evaluate_Causal_StoNet(data_seed, save_results=True):
    ...
    train_set = competing_risk_inter2(train_path, args.interval_length)
    test_set = competing_risk_inter2(test_path, args.interval_length)
    ...
    base_path = os.path.join(f'./competing_risk_results/cause_specific/complete_simple_inter/exp_{args.num_sim}', str(data_seed))
    ...
    for prune_seed in range(num_seed):
        ...
        with torch.no_grad():
            ...
            count = 0
            for target, x, actual_time, test_time, test_indicator in val_data:
    ...
    if save_results:
        with open(os.path.join(base_path, 'competing_stoNet_results' + str(data_seed) + '.json'), "w") as f:
            json.dump(results, f, indent=4)
    return results


for nnum in range(1,51,1):
    result = fit_and_evaluate_Causal_StoNet(nnum, save_results=True)
