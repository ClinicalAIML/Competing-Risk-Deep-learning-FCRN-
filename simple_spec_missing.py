...
import sys
sys.path.insert(0, '.')
from model.network_miss import StoNet_Survival
from model.training_missing import training_survival
from data import competing_risk_missing_inter
...
parser.add_argument('--num_sim', default=10, type=int, help='number of simulation runs')
parser.add_argument('--interval_length', default=5, type=int, help='number of simulation')
parser.add_argument('--miss_rate', default=0.25, type=float, help='missing rate for dataset')
parser.add_argument('--num_event', default=2, type=int, help='number of competing events')
#parser.add_argument('--train_epoch', default=300, type=int, help='total number of training epochs')
parser.add_argument('--learning_rate', default=1e-2, type=float, nargs='+',
...
parser.add_argument('-nbh', '--graph_nb', type=int, nargs='+', action='append',
...
# example: in terminal: -nbh 1 2  -nbh 1 2 3 4
# Parameter for StoNet
# model
parser.add_argument('--layer', default=3, type=int, help='number of hidden layers')
parser.add_argument('--unit', default=[16, 32, 16], type=int, nargs='+', help='number of hidden unit in each layer')
...
parser.add_argument('--impute_lr_miss', default=3e-3, type=float,
...
def fit_and_evaluate_Causal_StoNet(data_seed, save_results=True):
    ...
    train_path = f'./raw_data/interaction_data/miss_{args.miss_pattern}_{args.miss_rate}_train/train_data_{data_seed}.csv'
    test_path = f'./raw_data/interaction_data/complete_test/test_data_{data_seed}.csv'
    train_set = competing_risk_missing_inter(train_path, args.interval_length)
    test_set = competing_risk_missing_inter(test_path, args.interval_length)

    max_interval = train_set.get_max_interval()
    args.miss_col = train_set.get_miss_col()
    ...
    base_path = os.path.join(f'./competing_risk_results/cause_specific/missing_simple_inter_{args.miss_pattern}_{args.miss_rate}/exp_{args.num_sim}', str(data_seed))
    ...
    for prune_seed in range(num_seed):
        ...
        # train
        print("Train")
        output_train = training_survival(mode="train",
                                net=net,
        ...
        with torch.no_grad():
            ...
            count = 0
            for target, x, _, actual_time, test_time, test_indicator in val_data:
